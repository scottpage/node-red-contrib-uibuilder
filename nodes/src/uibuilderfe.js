        ...
        */
        ...
        self.setIOnamespace = function () {
            ...
            if (ioNamespace === '' ) {
                ...
                if (u.length > 0) {  // Check that u isn't zero (0) length, so endsWith doesn't fail
        ...
        /** Try to make sure client uses Socket.IO version from the uibuilder module (using path) @since v2.0.0 2019-02-24 allows for httpNodeRoot */
        {
            ...
            if (fullPath.length > 0) {  // Check that fullPath isn't zero (0) length, so endsWith doesn't fail
